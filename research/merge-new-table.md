...
- 开发库和生产库 `ainterview` 均已执行迁移；生产验收见第 10 节。数据库迁移不代表业务服务已经部署或重启。
...
1. 核对数据库身份、备份和恢复方案；保留源/目标基线。工具默认允许开发库；生产库写入须显式传入 `--database ainterview --confirm-database ainterview`，不能照抄开发库命令。
...
本次改造已通过 `next-version` 交付，并于 2026-09-07 快进合并、推送至 `dev`（`9a007f6e`），后续工作分支为 `dev`。拆分前提交为 `8d6b83338853f91a710df3c433d304923c43f6e5`。推送代码不执行数据库迁移、不部署服务。
...
用户授权将 `apps/web/.env` 指定的 `ainterview`（PostgreSQL 17.9）迁移至新模型。执行前最新迁移为 `20260904181000_meeting_live_summary`，新招聘表不存在；全部现有 schema 共 142 张表，其中 public 97 张。

### 备份与最终快照

私有材料目录：`/Users/wen/.local/state/codex-db-backups/recruiting-production-20260907/`。

- 全量备份 `before.dump` 为 91,425,273 字节，归档目录及整个归档解压读取均通过；SHA-256 为 `e4c95e73f3613a17f7880b6d99ea94a023014950749293bfdf171583de5cbdf7`。尚未在隔离数据库做完整恢复，归档读取校验不能称为恢复演练成功。
- 等待期间真人文档同步和邮箱账号发生变化，另保存 `after-shutdown-changed-tables.dump`。最终以事务锁内的 `locked-baseline-apply.json`、`locked-delta-apply.json` 和 `locked-source-cache.json` 为切换快照；缓存包含候选人原文，不进入 Git。
- 恢复应先在隔离库加载全量备份，再按锁内快照替换变化表，验证对应行数/哈希及约束。补充快照不是第二批可直接叠加插入的数据。新表启用后的增量必须另行保护，不能直接回退到旧系统。

### 执行中遇到的问题与处理

- 用户确认停写后仍观察到旧文档同步重试时间变化。已停止遗漏的本机 Vite 和 Agent；服务器 `42.194.218.213` 仍有旧消费者活动。用户进一步明确允许继续、无需处理该服务器。本次在事务内锁定数据取得最终快照，不将旧基线差异直接忽略。
- SHARE 锁在后续解除旧外键时发生锁升级死锁，事务完整回滚。改为事务开始时对 97 张原有 public 表一次取得 ACCESS EXCLUSIVE 锁，覆盖建表、复制、校验和依赖清理，提交/回滚时释放。
- 普通参数传输间歇性出现 JSON 缺字和 UTF-8 错误，本地失败批次本身是有效 JSON；缩小至 16 KiB 仍复现。一次性运维入口改用 Base64 参数，数据库会话内的临时函数解码并校验 MD5，语句保存点只对传输/解析异常作最多三次尝试。校验不一致拒绝写入，最终仍逐字段核对。临时函数不进入持久 schema。
- 带参数校验的完整演练捕获了一次传输校验不一致，保存点回滚重试后继续。复制、幂等复验、报告归档、旧依赖清理均成功，97 张原有 public 表行数和完整行 SHA-256 不变，整个演练最终主动回滚。
- 16 KiB 小批次只是排查措施，未能解决传输问题。工具保留原有 128 KiB／100 条上限；正式执行采用上述编码校验和原有批次，完整性检查不变。

### 验收状态

完整回滚演练及正式迁移均已通过，正式提交时间为 `2026-09-06T19:41:51.751Z`（北京时间 2026-09-07 03:41:51）。三个迁移及其 Drizzle 台账在同一事务提交。

| 验收项                     | 结果                                     |
| -------------------------- | ---------------------------------------- |
| 新表                       | 38 张业务表 + 1 张迁移台账               |
| 人才 / 简历 / 招聘记录     | 各 1,802 条，不合并人才                  |
| 节点状态                   | 14,416 条，全部招聘记录都有 8 个节点状态 |
| AI 轮次 / 真人轮次 / Offer | 199 / 65 / 7 条                          |
| 基础业务复制 / 迁移台账    | 各 64,892 条                             |
| 旧报告与版本归档           | 另 25 条事件                             |
| 提交前幂等复验             | 64,892 条全部一致，无新增或覆盖          |
| 原有 public 表             | 97 张在事务前后行数和完整行哈希一致      |
| 新表外键                   | 144 条，全部已验证                       |
| 旧档案与在线表之间的外键   | 0 条                                     |
| 旧主表自定义触发器         | 0 个                                     |
| 候选人详情真实只读投影     | 查询通过，读取 3 条、每条 77 个字段      |

提交后独立连接复验：64,892 条台账对应行全部存在。其中 64,888 条完整行哈希仍一致；4 条真人评价文档同步记录的 `next_attempt_at` 已被在线后台更新，与锁内快照逐字段比较确认只改变重试时间，未覆盖这些合法业务更新。不能再把旧源表回填工具当成实时同步入口。

历史会话的原始情况也已核对：源库有 51 条关联招聘记录、但轮次 ID 为空的会话；其中 1 条由唯一反向指针补齐，另外 50 条保持原有空关联和会话内容，不伪造轮次。它们不是这次拆表新产生的缺失。1 条历史待评估记录保留原状态和 runId，未自动重发；其任务恢复缺口属于已检查、尚未修改的业务问题。

脱敏执行证据：[生产迁移验收报告](../docs/reports/2026-09-07-recruiting-production-migration/verification.json)。原始执行报告、锁内数据快照和一次性运维脚本保留在私有备份目录。

生产目标保护和数据转换测试共 18 项通过，Server 类型检查及改动代码格式/lint 通过。此次仅处理数据库，不自动部署或重新启动业务服务；旧表观察期仍应从所有消费者切换新版本之后计算。
