...
- Job setup permits an optional Markdown “内部标准” field below the JD. Keep it private to recruiter editing, the job HoverCard, and recruiter evaluation agents; see `docs/adr/0040-use-private-job-internal-criteria.md`.
...
- The private free-text internal criteria field supersedes the JD-only input restriction in ADR 0029; it does not restore retired structured scoring controls.
...
- 不推荐 requires resume-supported conflict with an explicit job-related core JD or internal-criteria requirement. Sensitive personal attributes must not drive recommendations or rejection. Missing or conflicting evidence produces 待定, not 不推荐.
- Each of the six dimensions—技能匹配、经验相关性、项目匹配、教育与背景、潜力、稳定性—returns one of the same four advisory levels plus dense candidate-specific text. Prefer explicit job-related JD and internal-criteria requirements; when both are silent, apply the versioned general professional evidence standard and label the visible basis accordingly.
