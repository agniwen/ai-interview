...
Before editing TanStack Start or Router behavior, load the matching entry below once per task. Select by the API or behavior being changed; ordinary JSX, CSS, copy, and repository documentation edits do not need framework guidance. This catalog is the single maintained copy for both coding agents.
