...
`--strict` 要求每个案例至少运行 3 次。稳定性护栏为：结构成功率和允许行动命中率 100%、行动一致率至少 80%、维度区间命中率至少 90%、理由证据覆盖率至少 80%、总分波动不超过 10。单维最大波动仍会写入报告，用于观察单维离群，但不参与 `--strict` 门槛判定。案例没有配置维度区间或理由关键词时，对应指标显示为 `N/A` 且不参与门槛判定。这些是合成回归护栏，不是生产录用阈值。
