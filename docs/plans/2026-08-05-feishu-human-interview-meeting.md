...
>
> 2026-08-06 调整：飞书应用改由所选面试官的共同应用来源决定；两个应用都可用时优先 `feishu-jiguang-hr`。操作人不再进入飞书会议或日程，首位面试官担任 owner，全部面试官担任主持人和日程参与人。
>
...
1. 根据所选真人面试官的共同应用来源选择对应的飞书应用。
2. 以首位真人面试官为会议 owner，以全部真人面试官为主持人，创建一场飞书会议。
...
2. **按面试官共同来源选择飞书应用。**
   - 所选面试官只共同支持 `feishu` 时，使用第一个应用 `cli_a955211781785bd8`。
   - 共同支持 `feishu-jiguang-hr` 时，优先使用第二个应用 `cli_a97aa896aab85bc2`。
   - Google、密码登录或没有飞书绑定的面试官默认支持第二个应用。
   - 所选面试官没有共同应用来源时，前端禁选并由后端拒绝创建。
3. **应用身份创建。** 使用所选应用的 `tenant_access_token` 创建会议和日程，不要求用户重新进行飞书 OAuth 授权。
4. **仅真人面试官进入会议与日程。**
   - 第一位被选择的真人面试官作为飞书会议 `owner_id`。
   - 所有真人面试官加入 `assign_host_list`，作为会议主持人。
   - 所有真人面试官加入飞书日程参与人并去重；操作人不加入。
...
11. **身份解析采用严格策略。** 任一真人面试官无法解析为所选应用下的飞书用户时，不创建飞书资源，并明确返回无法解析的人员姓名。
...
### Feishu provider snapshot

飞书 `open_id` 是应用内身份，同一员工在两个飞书应用下的 `open_id` 不同。因此一次真人面试必须固定使用一个 provider，后续重试也必须复用该 provider。VC 预约创建前保存 owner 和全部面试官的 Open ID 快照；预约检查点存在后，Calendar 和参与人重试必须复用这些快照，不再读取当前账号绑定或通过邮箱重新解析。
...
请求体不增加由前端选择 provider 的字段；provider 必须由后端根据所选面试官的共同来源确定，避免客户端伪造或跨应用混用。
...
输入为真人面试官的本地用户记录，输出为指定 provider 下的 `open_id`。
...
调用添加日程参与人接口，将去重后的所有真人面试官以 `open_id` 添加。操作人不添加。
...
- 根据所选面试官的共同 provider 选择飞书应用；共同支持两个应用时优先第二个应用。
- 查询并校验所有所选面试官属于当前工作区。
- 校验所选面试官至少存在一个共同应用来源。
...
- 仅支持单个飞书 provider 的面试官选择正确 app ID。
- 同时支持两个 provider 时优先选择第二个 app。
- Google、密码和无飞书绑定的面试官默认选择第二个 app。
- 没有共同 provider 的面试官组合被拒绝。
- 跨工作区用户 ID 被拒绝。
- OAuth 账号优先于邮箱补查。
- 邮箱补查成功可继续。
...
2. 以真实面试官解析所选共同应用下的 `open_id`。
3. 创建一场短期测试 VC。
4. 在 Bot 主日历创建日程并挂载该 VC URL。
5. 验证所有面试官收到/可见日程，操作人未被加入。
...
- 所选面试官只共同支持飞书一时，记录固定使用应用 `cli_a955211781785bd8`。
- 所选面试官共同支持飞书二时，优先使用应用 `cli_a97aa896aab85bc2`。
- 没有飞书绑定的面试官默认使用应用 `cli_a97aa896aab85bc2`。
- 不存在共同应用来源的面试官组合在前端不可选，并由后端拒绝。
- 所有面试官都经过当前工作区成员校验，并解析为所选应用下的 `open_id`。
- 首位真人面试官是会议 owner；所有真人面试官是主持人和日程参与人；操作人不加入会议或日程。
