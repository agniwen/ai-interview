...
候选人短暂离线时保留同一个 agent 的内存状态，并通过既有 checkpoint/报告路径持久化答案；刷新页面恢复转写、计时和麦克风状态。Realtime 重连后等待候选人继续，避免再次生成上一条回答。房间 `departureTimeout` 设置为重连宽限期加 60 秒，确保 LiveKit 默认 20 秒房间回收不会提前结束会话。Qwen WebSocket 的可恢复断线会重建连接并恢复已确认的文本和工具历史；进程退出、未确认工具操作及重试耗尽仍会终止当前会话，不承诺跨进程恢复声学上下文。
...
| 连接故障 | 有界缓冲、超时和最多三次连续重连；恢复已确认历史，不重放音频或重新执行工具                             |
...
- 百炼可能以 `user_idle_timeout` / `response_idle_timeout` 终止空闲连接。适配器在没有未确认工具、配置或历史操作时，恢复指令、工具定义和已确认历史。正常空闲恢复不会重新开场；无未确认操作的自动回复超时也进入同一恢复路径。
- 用户或 AI 说话中途断线时，未完成音频不重放、不据此补全回答；恢复后提示候选人补说最后一句。协议要求的私有恢复信号不进入候选人回答历史。恢复通知通过 session.update 临时替换指令并关闭工具，播报完成后恢复原配置；不能仅依赖追加到历史的 system 消息。
- 未确认的工具执行、配置更新、手动音频提交，以及恢复提示本身再次断线，目前仍保守结束；不能宣称所有断线无损恢复。请求的 15 秒超时和响应的 120 秒看门狗是单次操作限制，不是整场面试时长上限。
- 上游连接的重建与候选人离开后的 180 秒宽限独立。手动结束立即生效，不延长到宽限期；获取重连 token 不清除离开期限；核实候选人与原 Agent 实际重连后清除旧断开时间，下一次离开独立计算 180 秒。
