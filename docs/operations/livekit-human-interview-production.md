...
- 模型返回超过 50 项、重复项或长度不合规的词时，先过滤、去重，再保留前 50 个合规原文术语，不再因超限丢弃整批提示。
...
### 2026-09-04 会后处理与实时字幕修复发布

这批修复需要重新构建并更新 **Web（`ai-tool-demo`）和 Worker（`ai-tool-worker`）**；如果使用独立 Server，也更新 Server。无需数据库迁移，无需重启或改动 LiveKit、Redis、Egress。Web 更新后，测试浏览器需刷新页面加载新的重连逻辑。

- 实时 ASR 的 `ServiceUnavailable` 或供应商容量限流会记录安全的错误码、任务 ID 和模型，不记录原始错误消息中的潜在令牌或输入内容。页面提示服务繁忙，以约 16–60 秒间隔退避重试；结束或暂停采集后取消重试。此处理不能消除供应商资源不足，也不能补齐断线期间的实时字幕，以已保存录音的会后转录为准。
- 智能分析及其招聘决定检查均向纯文本 JSON 模型提供完整输出 Schema；字段格式错误仍校验并反馈重试，不通过猜测角色或默认填值绕过校验。
- 评价明确区分转录词语待核实、材料覆盖不足与候选人的实质能力风险；前两者仅在详细分析中中性说明，不作为沟通能力、诚信或评级的负面依据。
- 评价生成后增加一次独立的模型证据复核；发现无依据负面判断时反馈并重新生成一次，再次不通过则不发布该草稿。仍保留有明确事实支持的负面评价，不自动抬高评级；会增加模型调用和处理时间，复核也不能替代面试官判断。
- 两方音轨成功转录、无已知失败，且全场录音晚于两路音轨启动时，开头不超过 1 秒的音轨启动间隙不再标记“处理失败”。这不表示该间隙被补录；更长缺口、真实录音/ASR 失败和身份不明仍保留提示。
- 已保存的旧转录、旧评价及失败智能分析不会因更新容器自动覆盖，需按原有重新生成流程处理。不要直接修改数据库状态来伪装成功。
