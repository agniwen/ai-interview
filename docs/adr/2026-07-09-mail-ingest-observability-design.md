...
mail_ingest_message（加列：skipReason/jdBindStatus/boundJobDescriptionId/extractedJobCodes/resumeAttachmentCount）
        │  1 邮件=1 message=1 batch=N 附件；batchId → batch item → pool item（各附件独立解析/入池状态）
        ▼
...
- `skipReason: text` —— `status=skipped` 时的原因，一期只有一个值：`no_supported_attachment`。
...
**基数（关键）**：`createBatchForMail` 每封信建**一个** batch，附件逐个建池条目 —— 即 **1 封邮件 = 1 个 message 行 = 1 个 batch = N 个附件（N 个 `resume_pool_item` / `resume_upload_batch_item`）**。`updateMailIngestMessageResult` 给 message 写**唯一** `batchId`，故 message↔batch 是 **1:1**、batch↔附件是 **1:N**。这是下游 JOIN 的稳定基础。

去重/解析/入池状态**不进 message 行**：由 UI 经 `message.batchId → resume_upload_batch_item → resume_pool_item`（`resumeParseStatus`/`resumeParseError`）与疑似重复表 JOIN 得到。一封信的 N 个附件各自有独立的解析/入池状态（见 §4 展开）。message 行只负责「入库前段（邮件级）」。

**关于 `already_processed`**：不设该 skipReason。同一封信的 message 行按 `uid` 唯一键**只创建一次**，其 `status` 反映真实终态（`queued`/`skipped`/`failed`）；后续轮询再遇到该 uid（`claimMailIngestMessageForProcessing` 命中现有终态行）**直接 no-op、不覆盖**——重复处理不是一个独立结局，无需记录。

### 2. 采集点改造（`apps/ai-recruitment-copilot-worker/src/mail-ingest/processor.ts`）

命中标题关键词、通过 `listenStart`、被 `claimMailIngestMessageForProcessing` **首次**认领的邮件 —— 建行，按路径写真实终态：
...
**JD 绑定判定（仅记录，不改绑定行为）**：本期只把当前绑定逻辑的**结果**写进 `jdBindStatus`/`extractedJobCodes`/`boundJobDescriptionId`，**不改变绑定动作本身**（歧义时是否回退、正则是否加前缀约束都归 #2）。决策表：
...
- **邮件行（每封信一行）**：收件时间 · 发件人 · 标题 · 附件数 · JD绑定（状态 + 岗位名）· 终态（`queued` | `skipped`+原因 | `failed`+错误）。
- **附件级展开（1:N，关键）**：邮件行可展开出该信 `batchId` 下的 **N 个附件/池条目**，每个显示：文件名 · 解析·入池状态（`resumeParseStatus` + `resumeParseError`）· 疑似重复标记 · →跳转（该池条目 / 若已入库则简历详情）。这样才能定位「同一封信里某一份简历为什么没解析/没入池」。
- 筛选：终态、`skipReason`、`jdBindStatus`、标题/发件人关键词、时间范围（**按 `receivedAt`**，列表默认也按 `receivedAt` 倒序）。
- 账号头部：§3 的上轮小结计数。
- 后端：**两个 surface 各有对应端点**，镜像现有账号端点的挂载与权限：
  ...
  - DAO 分页 + JOIN 下游（batch → batch item → pool item）。
- **权限/作用域**：账号是 `org+user` 维度。自助端只能看**自己创建（`createdBy=当前用户`）**账号的信件；平台端需 `mailIngestAccount` `manage` 权限、可看本 org 全部账号。共享邮箱 = 同一条 `mail_ingest_account` 行，可见性随该行的 `createdBy`。
...
- message 行随时间增长；**保留期/清理不在一期**，但表设计需为后续清理留约束：以 `receivedAt`/`createdAt` 为清理键，且清理不能破坏正在被引用的 `batchId` 关联（清理策略留待专项）。
- 下游 JOIN 只读，不改写业务状态。
- 「无附件」写行状态是**新增行为**，不改动既有幂等（uid 唯一键）、绑定动作与解析队列逻辑。

## 测试

- **worker（`processor` / `message-filter`）**：各路径 message 行的 `status`/`skipReason`/`jdBindStatus`/`resumeAttachmentCount` 正确 —— 成功入队、无附件跳过、入队失败、JD `bound/unmatched/ambiguous/fallback`（且**绑定动作未变**）；重复 uid 命中现有终态行 no-op 不覆盖；标题不匹配只累加 `subjectSkipped` 不建行；本轮小结（含 `lastRunReceived`）写回。
- **DAO**：`listAccountMailMessages` JOIN 下游状态（含**一封信 N 附件各自 `resumeParseStatus`**）、过滤（终态/原因/JD状态/关键词/`receivedAt` 时间）、分页、作用域（`createdBy`）。
