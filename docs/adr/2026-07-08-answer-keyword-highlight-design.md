...
- **metric**（数字/绩效）：顶层常量正则,四类分支：
  - **阿拉伯数字紧跟单位/百分号**（数字与单位间**不允许空格**）：百分比 `30%`/`30％`；量级前缀 `500万`/`3亿`/`5千`（可再接单位如 `500万元`）；带单位计数 `10人`/`5个`/`3年`/`2倍`/`3次`/`2名`；金额 `500元`/`500万元`；小数 `3.5年`。
  - **货币符号前缀**：`￥500`/`¥500`。
