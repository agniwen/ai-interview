...
The qualitative resume-evaluation contract replaces configurable scoring for new evaluation attempts, but every existing result remains stored and readable under the contract version that produced it. Scores, gates, and adjustments are not mapped into the new four-level recommendation because the contracts are not semantically equivalent; a recruiter must explicitly request a reassessment before a candidate receives a qualitative result, and a failed reassessment retains the last valid result. Qualitative V2 adds the same four-level recommendation to every dimension; V1 results remain readable without inferred dimension levels and require reassessment before a six-dimension chart can be shown.
...
Candidate cards and the Overview tab show the recommendation level plus the concise overall evaluation; “view details” opens the AI Evaluation tab, which directly shows the detailed overall evaluation, six dimension levels and narratives, and any supported seniority or team-positioning guidance. Detailed narrative fields may use restricted Markdown containing only bold, italic, ordered lists, and unordered lists; headings and all other Markdown or HTML constructs are forbidden by the prompt and removed by the Typeset rendering whitelist. Overview and AI Evaluation retain the six-dimension radar chart by mapping the ordered qualitative levels to radial positions; those rendering positions are not exposed as scores, weighted, or summed. The recommendation uses not recommended with a red X, undecided with a yellow question mark, recommended with a green thumbs-up, and highly recommended with a purple sparkle. Text labels are always present so color is never the only signal.
