...
const backendRoot = path.join(repoRoot, "apps/server/src");
...
describe("request-scoped workspace authorization boundary", () => {
  ...
  it("centralizes Better Auth permission checks that require an explicit organization", () => {
    expect(filesContaining(backendRoot, /auth\.api\.hasPermission/u)).toEqual([
      "apps/server/src/server/access/workspace-permissions.ts",
    ]);
  ...
  it("keeps recruiting group resource and role policy in one access package", () => {
    ...
    expect(filesContaining(backendRoot, policyPattern).toSorted()).toEqual(
      [
        "apps/server/src/server/access/recruiting-group-access.ts",
        "apps/server/src/server/access/workspace-permission-snapshot.ts",
      ].toSorted(),
    );
