...
import { enqueueAiInvitationResponseEvent } from "../../interview-notifications/utils/events";
    ...
    factory
      ...
      .post("/:id/:roundId/livekit-token", async (c) => {
        ...
        const resolution = await db.transaction(async (tx): Promise<TokenResolution> => {
          ...
          // 过期后保留原房间关联，允许迟到报告仍归属该轮次。
          // Expired grace is terminal; preserved anchors never authorize a new dispatch.
          ...
          if (row.liveKitRoomName && row.liveKitParticipantIdentity) {
            // Token prewarming may precede the first join. After the bootstrap window,
            // only the existing agent may be resumed; an absent room is terminal.
            const bootstrap =
              row.status === "in_progress" &&
              row.sessionStartedAt &&
              now.getTime() - row.sessionStartedAt.getTime() < RECONNECT_GRACE_MS;
            if (!bootstrap) {
              const liveSession = await dependencies.getLiveSession(row.liveKitRoomName);
              if (liveSession === "unavailable") {
                return { status: "session_unavailable" };
              }
              if (liveSession === "ended") {
                await tx
                  .update(aiInterviewRound)
        ...
        try {
          ...
          return c.json(
            { isReconnect, participantName, participantToken, roomName, serverUrl },
            200,
          );
        } catch (error) {
          return c.json(
      ...
      .post(
        ...
        async (c) => {
          ...
          const roundId = c.req.param("roundId");
          const mode = c.req.valid("query").mode ?? "interrupt";
          const now = new Date();
          ...
          const result = await db.transaction(async (tx) => {
            ...
            if (mode === "agent") {
              if (
                ...
                (await dependencies.getLiveSession(entry.liveKitRoomName)) !== "ended"
              ) {
                return { kind: "not_ended" } as const;
              }
              await tx
                .update(aiInterviewRound)
                .set({ status: "completed", updatedAt: now })
