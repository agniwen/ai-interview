...
} from "../utils/human-interview-evaluation-state";

const humanInterviewTranscriptionStateSchema = z.enum(["failed", "pending", "processing", "ready"]);

export async function listHumanInterviewEvaluationSnapshotsForAnalysis(input: {
...
export type HumanInterviewLiveTranscriptRecoveryResult =
  | { meetingSessionId: string; status: "ready"; transcriptRevisionId: string }
  | { status: "meeting-not-ended" }
  | { status: "no-live-transcript" }
  | { status: "not-found" };

export async function recoverHumanInterviewReviewFromLiveTranscript(input: {
  actorId: string;
  meetingId: string;
  organizationId: string;
  roundId: string;
...
export async function loadHumanInterviewReview(input: {
  meetingId: string;
  organizationId: string;
  roundId: string;
  ...
  const transcript =
    row.meetingSessionId && row.activeTranscriptRevisionId
      ? await loadMeetingTranscriptRevision({
...
export async function requestHumanInterviewEvaluation(input: {
  force: boolean;
...
export async function claimHumanInterviewEvaluationAfterTranscriptCorrection(input: {
  meetingSessionId: string;
  organizationId: string;
  roundId: string;
}): Promise<HumanInterviewEvaluationJobData | null> {
...
export async function listRecoverableHumanInterviewEvaluationJobs(): Promise<
...
export async function loadHumanInterviewEvaluationInput(
...
export async function publishHumanInterviewEvaluation(input: {
...
export async function markHumanInterviewEvaluationFailed(input: {
  error: string;
  roundId: string;
  transcriptRevisionId: string;
}): Promise<void> {
...
export async function saveHumanInterviewEvaluationDraft(input: {
  actorId: string;
  evaluation: HumanInterviewEvaluation;
  meetingSessionId: string;
  organizationId: string;
...
export async function submitHumanInterviewEvaluation(input: {
  actorId: string;
  evaluation: HumanInterviewEvaluation;
  meetingSessionId: string;
  organizationId: string;
