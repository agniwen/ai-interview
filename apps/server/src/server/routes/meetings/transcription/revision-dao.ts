...
async function serializeTranscriptRevision(
  ...
  return {
    ...
    provider: meetingTranscriptRevisionProviderSchema.parse(revision.provider),
...
export async function createInitialHumanMeetingTranscriptRevision(input: {
  actorId: string;
  meetingId: string;
  organizationId: string;
  text: string;
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const [activeRevision] = await tx
      ...
      .limit(1);
    if (meeting.activeTranscriptRevisionId && !activeRevision) {
      return { status: "conflict" as const };
    }
    const [sourceRun] = await tx
      .select({
        ...
        region: meetingProcessingRun.region,
      })
      ...
      .where(
        and(
          ...
          eq(meetingProcessingRun.stage, "final-transcription"),
        ),
      )
    ...
    const durationMs = Number(duration?.durationMs ?? 0);
    const source = activeRevision ??
      sourceRun ?? {
        model: "manual-input",
        pipelineVersion: "human-manual-v1",
        provider: "manual",
        region: "local",
      };
    if (durationMs <= 0) {
    ...
    const [revision] = await tx
      .insert(meetingTranscriptRevision)
      .values({
        basedOnRevisionId: activeRevision?.id ?? null,
        createdBy: input.actorId,
        id: revisionId,
        kind: "human",
        language: "zh-CN",
    ...
    await tx.insert(meetingTranscriptTurn).values(turns);
    if (meeting.intelligenceRunId) {
      await tx
        .update(meetingProcessingRun)
        .set({
          errorCode: "superseded",
          errorMessage: "Authoritative transcript was created manually",
          executionToken: null,
          finishedAt: new Date(),
          status: "failed",
        })
    ...
    await tx.insert(meetingAuditLog).values({
      ...
      detail: {
        ...
        revisionId,
        sourceRevisionId: activeRevision?.id ?? null,
      },
      id: crypto.randomUUID(),
      meetingId: input.meetingId,
      organizationId: input.organizationId,
...
export async function listMeetingTranscriptRevisions(input: {
  ...
  return rows.map((row) => ({
    ...
    model: row.model,
    provider: meetingTranscriptRevisionProviderSchema.parse(row.provider),
    region: row.region,
