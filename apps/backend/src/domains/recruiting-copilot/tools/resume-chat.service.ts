...
export class ResumeChatService {
  ...
  async chat(context: WorkspaceAuthorizationContext, input: ResumeChatInput) {
  ...
  private async resolveVisibility(
    ...
    const userIds = await this.recruitingScope.visibleCreatorIds(
      context.workspace.id,
      context.actor.id,
      context.member.role,
    );
    return userIds === null ? { kind: "all" } : { kind: "restricted", userIds };
  }
}
