...
import { InvalidJoinLinkError, JOIN_EFFECTS_PORT } from "./join.port.js";
import type { JoinAcceptResult, JoinPreview, JoinEffectsPort, JoinPort } from "./join.port.js";

@Injectable()
export class JoinService implements JoinPort {
  constructor(
    @Inject(HTTP_DATABASE)
    private readonly database: HttpDatabase,
    @Inject(JOIN_EFFECTS_PORT)
    private readonly effects: JoinEffectsPort,
  ) {}
