...
import { createMeetingTranscriptionProcessorPorts } from "../background-workloads/processors/meeting-transcription.processor.js";
import type {
...
export class MeetingTranscriptionInfrastructure {
  ...
  private readonly intelligenceRecovery: MeetingIntelligenceRecoveryService;
  ...
  constructor(
    ...
    this.intelligenceRecovery = intelligenceRecovery;
    this.env = env;
  }
  ...
  private async requestIntelligence(_input: { meetingId: string; organizationId: string }) {
    await this.intelligenceRecovery.recoverMissing();
  }
}
