...
export interface IntelligenceTranscriptTurn {
  endMs: number;
  id: string;
...
export interface MeetingIntelligenceGenerationClient {
  generate(input: {
    maxOutputTokens: number;
    prompt: string;
    purpose: "decision-policy" | "payload";
  }): Promise<object>;
}

export interface MeetingIntelligenceGenerationRuntime {
...
function templateInstructions(template: MeetingIntelligenceTemplate): string {
  return template === "general" ? GENERAL_TEMPLATE_INSTRUCTIONS : RECRUITING_TEMPLATE_INSTRUCTIONS;
}
...
- 每一条结构化条目必须填写至少一个 evidenceTurnIds；
...
function fittingTurnTextPrefixLength(
  ...
  while (low <= high) {
    ...
    if (serializedTurnsLength([{ ...turn, text: text.slice(0, candidateLength) }]) <= maxChars) {
  ...
  for (const turn of turns) {
    if (serializedTurnsLength([turn]) > maxChars) {
      ...
      while (offset < turn.text.length) {
        ...
        const length = fittingTurnTextPrefixLength(turn, remaining, maxChars);
        chunks.push([{ ...turn, text: remaining.slice(0, length) }]);
        offset += length;
      }
      continue;
    }
...
async function generatePayload(input: {
  client: MeetingIntelligenceGenerationClient;
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (
        ...
        !validateMeetingIntelligenceEvidence(payload, input.evidenceTurnIds)
      ) {
        throw new Error("Meeting Intelligence evidence 或模板与输入不一致");
      }
      return payload;
    } catch (error) {
      lastError = error;
  ...
  throw new MeetingIntelligenceTerminalError(
    lastError instanceof Error ? lastError.message : "Meeting Intelligence 结构化输出无效",
  );
}
...
async function reducePayloads(input: {
  ...
  while (source.length > 1) {
    ...
    for (let index = completed.length; index < groups.length; index += 1) {
      ...
      await persistProgress(input.runtime, { ...input.progress, completed, source });
    }
...
async function assertRecruitingDecisionPolicy(
  ...
  const decision = decisionPolicyResultSchema.parse(
    await client.generate({
      maxOutputTokens: 500,
      prompt: `判断下面的 Recruiting Interview intelligence 是否包含系统对候选人的招聘决定或建议。录用、拒绝、淘汰、通过、不通过、推进至下一轮均分类为 hiring-decision；仅复述事实分类为 allowed。\n\nIntelligence JSON：\n${JSON.stringify(content)}`,
      purpose: "decision-policy",
    }),
  );
...
export async function generateMeetingIntelligence(
  ...
  const chunks = splitTurns(input.turns, maxTranscriptChars);
  ...
  if (progress.phase === "map") {
    ...
    for (let index = progress.completed.length; index < chunks.length; index += 1) {
      ...
      const content: MeetingIntelligencePayload = await generatePayload({
        client,
      ...
      progress = { ...progress, completed: [...progress.completed, content] };
      await persistProgress(runtime, progress);
    }
  }
  ...
  await assertRecruitingDecisionPolicy(content, client, runtime.heartbeat);
  return content;
}
