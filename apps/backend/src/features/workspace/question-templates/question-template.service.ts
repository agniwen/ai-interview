...
import { refreshEligibleCommunicationQuestions } from "./refresh-eligible-candidates.js";
...
export class QuestionTemplateService {
  ...
  async refreshEligibleCandidates(organizationId: string, operatorId: string, templateId: string) {
    try {
      const result = await refreshEligibleCommunicationQuestions(this.database, {
        operatorId,
        organizationId,
        templateId,
      });
