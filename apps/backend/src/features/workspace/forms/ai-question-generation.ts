...
function parseAiProviderContent(payload: z.infer<typeof aiProviderResponseSchema>) {
...
export async function requestWorkspaceAiJson(prompt: string): Promise<JsonValue> {
  const { apiKey, baseUrl, model } = aiProviderConfig();
  ...
  return parseAiProviderContent(aiProviderResponseSchema.parse(await response.json()));
}
...
export async function generateCandidateFormQuestions(
  ...
  const result = formGenerationSchema.parse(await requestWorkspaceAiJson(prompt));
...
export async function generateCommunicationQuestions(
  ...
  const result = interviewGenerationSchema.parse(await requestWorkspaceAiJson(prompt));
