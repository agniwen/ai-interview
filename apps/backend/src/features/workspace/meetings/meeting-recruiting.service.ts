...
import { WORKSPACE_ACCESS_PORT, WORKSPACE_DATABASE_PORT } from "../workspace.ports.js";
import type {
  WorkspaceAccessPort,
...
export class MeetingRecruitingService {
  ...
  private async meeting(context: WorkspaceRequestContext, meetingId: string) {
  ...
  private async canRead(context: WorkspaceRequestContext) {
    return this.access.authorize(context, { action: "read", resource: "resumeLibrary" });
  }

  private async visibleCreatorIds(context: WorkspaceRequestContext): Promise<string[] | null> {
  ...
  async get(context: WorkspaceRequestContext, meetingId: string) {
