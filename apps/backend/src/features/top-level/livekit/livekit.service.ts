...
import { TOP_LEVEL_LIVEKIT_HUMAN_MEETING_PORT } from "./livekit.port.js";
import type { TopLevelLiveKitHumanMeetingPort, TopLevelLiveKitPort } from "./livekit.port.js";
...
@Injectable()
export class LiveKitService implements TopLevelLiveKitPort {
  private receiver: WebhookReceiver | null = null;

  constructor(
    @Inject(TOP_LEVEL_DATABASE_PORT)
    private readonly database: TopLevelDatabasePort,
    @Inject(TOP_LEVEL_LIVEKIT_HUMAN_MEETING_PORT)
    private readonly humanMeetings: TopLevelLiveKitHumanMeetingPort,
  ) {}
