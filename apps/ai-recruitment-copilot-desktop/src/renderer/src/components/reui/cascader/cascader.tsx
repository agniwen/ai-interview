  ...
  return (
    ...
    >
      ...
      {showIcon ? (
        <Icon
          icon="ph:caret-down"
          className={`${TRIGGER_ICON_CLASS} pointer-events-none shrink-0`}
        />
      ) : null}
    </ComboboxPrimitive.Trigger>
  );
...
/** The floating panel's surface. The shared popup motion comes from the
 * renderer stylesheet; sizing remains owned by `CascaderContent`. */
const CONTENT_SURFACE_CLASS =
  "t-dropdown bg-popover text-popover-foreground overflow-hidden ring-1 ring-foreground/10 shadow-md rounded-lg";
 ...
 */
// Use the project's native viewport path so the virtualizer can bind before paint.
const CASCADER_VIEWPORT_PROPS = { "data-slot": "scroll-area-viewport", tabIndex: -1 };
...
function CascaderList({ className, style, maxHeight: maxHeightProp, ...props }: CascaderListProps) {
  ...
  return (
    ...
    >
      ...
      >
        <ScrollArea className={CASCADER_SCROLL_CLASS} viewportProps={CASCADER_VIEWPORT_PROPS}>
