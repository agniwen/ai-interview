...
const JOB_CONTEXT_TEMPLATE = "__template_jd__";
...
export function InterviewQuestionTemplateAiGeneratePanel({
  jobDescriptionIds,
  jobDescriptions,
  onGenerated,
  templateDescription,
  ...
  onGenerated: (questions: InterviewQuestionTemplateQuestionInput[]) => void;
  templateDescription: string;
  templateScope: "global" | "job_description";
  ...
  const [jobDescriptionId, setJobDescriptionId] = useState<string>(JOB_CONTEXT_TEMPLATE);
  const [generating, setGenerating] = useState(false);

  const defaultJobContext = useMemo(() => {
    if (templateScope === "job_description" && jobDescriptionIds.length > 0) {
      return JOB_CONTEXT_TEMPLATE;
    }
    if (selectedCandidateIds.length > 0) {
      return JOB_CONTEXT_CANDIDATE;
    }
    return JOB_CONTEXT_TEMPLATE;
  }, [jobDescriptionIds.length, selectedCandidateIds.length, templateScope]);
  ...
  const jobContextOptions = useMemo(() => {
    ...
    if (templateScope === "job_description" && jobDescriptionIds.length > 0) {
      options.push({ label: "使用模板绑定的岗位", value: JOB_CONTEXT_TEMPLATE });
    }
    if (selectedCandidateIds.length > 0) {
    ...
    return options;
  }, [jobDescriptionIds.length, jobDescriptions, selectedCandidateIds.length, templateScope]);
  ...
  async function handleGenerate() {
    ...
    try {
      const resolvedJobId =
        jobDescriptionId === JOB_CONTEXT_TEMPLATE || jobDescriptionId === JOB_CONTEXT_CANDIDATE
          ? undefined
          : jobDescriptionId;
      ...
      const result = await rpcFetch<{ questions: InterviewQuestionTemplateQuestionInput[] }>(
        rpc.api.w[":slug"].studio["interview-questions"]["ai-generate-questions"].$post({
          json: {
            ...
            jobDescriptionIds:
              templateScope === "job_description" && jobDescriptionId === JOB_CONTEXT_TEMPLATE
                ? jobDescriptionIds
                : undefined,
            prompt: prompt.trim(),
      ...
      toast.success(`已生成 ${result.questions.length} 道面试题，可在下方编辑`);
    } catch (error) {
  ...
  return (
    <div className="rounded-lg border border-dashed border-primary/30 bg-primary/5 p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <p className="font-medium text-sm">AI 智能生成面试题</p>
      </div>
      <p className="mb-4 text-muted-foreground text-xs">
        根据候选人、岗位信息与填写指令，自动生成语音面试必问题，并填入下方题目列表。
      </p>
