...
function getDefaultPdfDocumentOptions(pdfjsVersion: string): ReactPdf.DocumentProps["options"] {
...
function PDFViewerLoadingSkeleton({
  ...
  return (
    <div className="absolute inset-0 z-20 flex bg-muted/30">
      {sidebarOpen ? <DocumentViewerSidebarSkeleton inline={sidebarInline} /> : null}
...
function normalizeRenderRange(range: PDFViewerRenderRange, renderedPageNumbers: number[]) {
  const startPage = clampPageNumber(Math.min(range.startPage, range.endPage), renderedPageNumbers);
  const endPage = clampPageNumber(Math.max(range.startPage, range.endPage), renderedPageNumbers);
...
function PDFViewerPage({
  ...
  }, [clearSearchHighlights, hasSearchQuery, shouldRenderPage, updateSearchHighlights]);
...
export const PDFViewer = React.forwardRef<PDFViewerHandle, PDFViewerProps>(function PDFViewer(
  ...
  const [pageMetrics, setPageMetrics] = React.useState<Map<number, PDFPageMetrics>>(
    () => new Map(),
  );
  ...
  const [settledPageNumbers, setSettledPageNumbers] = React.useState<Set<number>>(() => new Set());
  const [lowResolutionPageNumbers, setLowResolutionPageNumbers] = React.useState<Set<number>>(
    () => new Set(),
  );
  ...
  const getEstimatedPageSize = React.useCallback(
    (index: number) => {
      ...
      const rotationDelta = pageRotationDeltas.get(pageNumber) ?? 0;
      const effectiveRotation = normalizeRotation(metrics.rotation + rotationDelta);
      const dimensions = getPageDimensions(metrics, effectiveRotation);

      return dimensions.height * zoom + PAGE_VIRTUALIZER_GAP;
    },
    [firstRenderedPage, getPageMetrics, pageRotationDeltas, renderedPageNumbers, zoom],
  );
  ...
  const maxEstimatedPageWidth = React.useMemo(() => {
    ...
    return renderedPageNumbers.reduce(
      (maxWidth, pageNumber) => Math.max(maxWidth, getEstimatedPageWidth(pageNumber)),
      0,
    );
    ...
    async (pageNumbersToLoad: number[]) => {
      ...
      const pageMetricEntries = await Promise.all(
        uniquePageNumbers.map(async (pageNumber) => {
          try {
            return [pageNumber, await getPdfPageMetrics(pdf, pageNumber)] as const;
          } catch {
            return null;
          }
        }),
  ...
  const defaultDocumentOptions = React.useMemo(
    () => (reactPdf ? getDefaultPdfDocumentOptions(reactPdf.pdfjs.version) : undefined),
    [reactPdf],
  );
  ...
  const getEstimatedThumbnailItemSize = React.useCallback(
    (index: number) => {
      ...
      return (
        THUMBNAIL_WIDTH / thumbnailAspectRatio + THUMBNAIL_ITEM_CHROME_HEIGHT + THUMBNAIL_ITEM_GAP
      );
    },
    [firstRenderedPage, getPageMetrics, pageRotationDeltas, renderedPageNumbers],
  );
  ...
  const renderRange = React.useMemo(() => {
    ...
    const slowTrailingBuffer =
      scrollState.direction === "none" ? 0 : Math.min(2, normalizedPageRenderBuffer);
    ...
    const afterBuffer = scrollState.isFastScrolling
      ? scrollState.direction === "forward"
        ? fastAheadBuffer
        : fastBehindBuffer
      : normalizedPageRenderBuffer + (scrollState.direction === "forward" ? slowTrailingBuffer : 0);
    ...
    return normalizeRenderRange(customRange ?? defaultRange, renderedPageNumbers);
  }, [
    activePage,
    normalizedPageRenderBuffer,
    numPages,
  ...
  const queuedPageNumbers = React.useMemo(() => {
    ...
    const visibleUnsettledCandidates = virtualPageNumbers.filter(
      (pageNumber) => visiblePageNumbers.has(pageNumber) && !settledPageNumbers.has(pageNumber),
    );
    ...
    return new Set(
      [...visibleUnsettledCandidates, ...unsettledCandidates].slice(0, maxUnsettledPages),
    );
  }, [
    activePage,
    renderRange.endPage,
  ...
  const pageNumbersForMetrics = React.useMemo(() => {
    ...
    renderedPageNumbers.forEach((pageNumber) => {
      if (pageNumber >= renderRange.startPage && pageNumber <= renderRange.endPage) {
  ...
  const updateActivePageFromViewport = React.useCallback(() => {
    ...
    viewport.querySelectorAll<HTMLElement>("[data-pdf-viewer-page]").forEach((pageElement) => {
      ...
      if (pageRect.bottom > viewportRect.top && pageRect.top < viewportRect.bottom) {
    ...
    const handleScroll = () => {
      ...
      if (direction !== "none") {
        setScrollState((currentState) =>
          currentState.direction === direction && currentState.isFastScrolling === isFastScrolling
            ? currentState
  ...
  React.useImperativeHandle(
    ...
    () => ({
      ...
      scrollToPageArea: (pageNumber, area, options) => {
        const viewport = viewportRef.current;
        const pageIndex = renderedPageNumbers.indexOf(pageNumber);
        const pageOffset = pageVirtualizer.getOffsetForIndex(pageIndex, "start")?.[0];
        ...
        const effectiveRotation = normalizeRotation(metrics.rotation + rotationDelta);
        const dimensions = getPageDimensions(metrics, effectiveRotation);
        const targetTop = pageOffset + (area.top / 100) * dimensions.height * zoom - 96;
    ...
    [getPageMetrics, pageRotationDeltas, pageVirtualizer, renderedPageNumbers, zoom],
  );
  ...
  const scrollToPage = React.useCallback(
    ...
    [onActivePageChange, pageVirtualizer, renderedPageNumbers, restoreWindowScroll],
  );
  ...
  React.useEffect(() => {
    ...
    const frameId = window.requestAnimationFrame(scrollActiveThumbnailIntoView);
  ...
  const handlePageSettled = React.useCallback(
    (pageNumber: number, devicePixelRatioLimit: number) => {
      ...
      setLowResolutionPageNumbers((currentPageNumbers) => {
        const isLowResolutionRender = devicePixelRatioLimit < MAX_DEVICE_PIXEL_RATIO;
  ...
  const rotateActivePage = React.useCallback(
    (rotationDelta: number) => {
      setPageRotationDeltas((currentRotations) => {
        ...
        const nextRotation = normalizeRotation(currentRotation + rotationDelta);
  ...
  }, [isPreparingDownload, pageRotationDeltas, pdfFile, resolvedDownloadFileName]);
              ...
              <DocumentViewerThumbnailSidebar inline={sidebarInline} open={thumbnailSidebarVisible}>
                {thumbnailSidebarVisible ? (
                  <div className="h-full overflow-auto" ref={thumbnailViewportRef}>
                    ...
                    >
                      {thumbnailVirtualizer.getVirtualItems().map((virtualThumbnail) => {
                        const pageNumber = renderedPageNumbers[virtualThumbnail.index];
                        ...
                        const thumbnailDimensions = getPageDimensions(metrics, effectiveRotation);
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    width: maxEstimatedPageWidth + PAGE_VIRTUALIZER_PADDING * 2,
                  }}
                >
                  {virtualPageItems.map((virtualPage) => {
                    ...
                    const shouldRenderPage =
                      settledPageNumbers.has(pageNumber) || queuedPageNumbers.has(pageNumber);
                    const devicePixelRatioLimit =
                      (scrollState.isFastScrolling && !settledPageNumbers.has(pageNumber)) ||
                    ...
                    const pageSearchQuery =
                      searchQuery.trim() && Math.abs(pageNumber - activePage) <= 1
                        ? searchQuery
                        : "";
