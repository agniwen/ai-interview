...
describe("Hono app factory boundary", () => {
  ...
  it("lets the Next route handler construct the app inside the Next adapter layer", () => {
    const source = readSource("app/api/[[...route]]/route.ts");

    expect(source).toContain("createServerApp");
    expect(source).not.toContain('import { app } from "@/server/app"');
  });
});
