...
- get_resume_record_detail 返回的任一候选人有 jobDescriptionId 时，前端会主动展示该候选人的数据库评价卡。若 resumeEvaluationArtifactMode 为 qualitative 且 qualitativeResumeEvaluation 不为空，必须优先采用新版六维定性评价，结合 recommendationLevel、conciseOverall、detailedOverall 和六维 dimensions 回答，不得把四级评价转换成数字分数。否则若 resumeEvaluationArtifactMode 为 structured 且 structuredResumeReview 不为空，采用历史结构化评分；否则若 resumeReview 不为空，采用历史六维评分。当前模式对应的评价不为空而 resumeReview 为空是正常状态，绝不能据此声称“尚未生成”。只有 qualitativeResumeEvaluation、structuredResumeReview 与 resumeReview 三类结果都为空时，才根据 resumeReviewStatus 说明未生成、生成中或失败。任何模式都不要自行重新估分。
