...
import { meetingQuestionMessagesRouter } from "./routes/messages/route";

export const meetingQuestionsRouter = factory
  .createApp()
  .get("/", async (c) => {
    ...
    const records = await listSavedMeetingQuestionThreads({
      meetingId,
  ...
  .post(
    ...
    async (c) => {
      ...
      const result = await createSavedMeetingQuestionThread({
        meetingId,
        memberRole: member.role,
        organizationId: activeOrg.id,
  ...
  .route("/:threadId/messages", meetingQuestionMessagesRouter)
  .get("/:threadId", async (c) => {
    ...
    const result = await getSavedMeetingQuestionThread({
      meetingId,
      memberRole: member.role,
      organizationId: activeOrg.id,
    ...
    return result ? c.json(result, 200) : c.json({ error: "Meeting Question thread 不存在" }, 404);
  });
