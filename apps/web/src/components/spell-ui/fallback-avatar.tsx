...
function oklchToRgb(L: number, C: number, H: number): [number, number, number] {
  ...
  const B = -0.004_196_086_3 * l - 0.703_418_614_7 * m + 1.707_614_701 * s;
  const gamma = (v: number) => (v <= 0.003_130_8 ? 12.92 * v : 1.055 * v ** (1 / 2.4) - 0.055);
  return [
...
function ensureGL() {
  ...
  const vShader = _gl.createShader(_gl.VERTEX_SHADER);
  if (!vShader) {
    return;
  }
  _gl.shaderSource(vShader, VERT_SRC);
  ...
  _gl.attachShader(_prog, vShader);
  _gl.attachShader(_prog, fs);
  _gl.linkProgram(_prog);
}

function renderToCanvas(targetCanvas: HTMLCanvasElement, uniforms: Uniforms, time: number) {
  ensureGL();
  if (!(_gl && _glCanvas && _prog && _qBuf)) {
    return;
  }
