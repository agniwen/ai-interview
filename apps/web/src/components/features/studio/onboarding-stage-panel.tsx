...
function OnboardingHiredDetails({
  ...
  return (
    <>
      <DetailRow label="实际入职日" value={details?.actualJoiningDate || <EmptyValue />} />
...
export function OnboardingStagePanel({ record }: { record: ResumeLibraryDetail }) {
      ...
      <FramePanel className="space-y-6">
        {!closed && (
          <p className="text-muted-foreground text-sm">
            跟进入职材料和到岗情况。候选人到岗后，点击操作栏的“确认入职结果”，填写说明并确认入职。
          </p>
        )}
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <DetailRow
            label="最早可入职日"
            value={record.candidateExpectationsMeta?.earliestJoiningDate || <EmptyValue />}
          />
          {closed && (
            <DetailRow
              label="结果确认时间"
