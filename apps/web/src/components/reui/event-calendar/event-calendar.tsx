...
function createEventCalendarStore<TData>(
  ...
  const api: EventCalendarApi<TData> = {
    ...
    findOverlapping({ start, end, excludeEventId }) {
      return api.getOccurrences({ end, start }).filter((occ) => occ.eventId !== excludeEventId);
    },
    getActiveRange() {
    ...
    getOccurrencesForDay(day) {
      ...
      for (const seg of [...bucket.allDay, ...bucket.timed]) {
        if (seen.has(seg.occurrence.key)) {
          continue;
        }
...
const VIEW_CONFIG_KEYS: (keyof EventCalendarViewConfig)[] = [
  "scrollToHour",
  "nowIndicator",
  "interval",
  "maxEventsPerCell",
...
const OPTION_KEYS: (keyof UseEventCalendarStateOptions)[] = [
  "events",
  "defaultEvents",
  "view",
  "defaultView",
