...
import type { EventCalendarInstance } from "@/components/reui/event-calendar/event-calendar";
import {
  snapMinutes,
  toZoned,
  zonedStartOfDay,
...
function beginBlockedGesture<TData>(
  ...
  const { event } = segment.occurrence;
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
  ...
  const computeProposal = (
    ...
    if (kind === "create") {
      if (config.createAllDay || surface.columns.length === 0) {
        ...
        return { allDay: true, dayGranular: true, end, start };
      }
      const col = findColumn(surface, startX);
      if (!col) {
        return null;
    ...
    const occStartMinInCol = Math.round((occurrence.start.getTime() - dayStart.getTime()) / 60_000);
    const occEndMinInCol = Math.round((occurrence.end.getTime() - dayStart.getTime()) / 60_000);
    if (kind === "resize-start") {
      const clamped = Math.min(
      ...
      return { allDay: false, end: occurrence.end, start };
    }
    ...
    return { allDay: false, end, start: occurrence.start };
  };
  ...
  const applyProposal = (e: PointerEvent) => {
    ...
    if (kind === "create") {
      ...
      if (settings.canSelectSlot && !settings.canSelectSlot(draft)) {
        return;
      }
  ...
  let carryDX = 0;
  let carryDY = 0;
  const CARRY_CLASS = `bg-background pointer-events-none fixed top-0 left-0 z-100 overflow-hidden rounded-sm opacity-90 shadow-md will-change-transform${
    ui?.dragCarry ? ` ${ui.dragCarry}` : ""
  }`;
  const CARRY_INVALID_CLASS = `${CARRY_CLASS} ring-destructive/60 ring-1${
  ...
  const createCarry = () => {
    ...
    carryEl.dataset.slot = "event-calendar-drag-carry";
    ...
    carryEl.append(clone);
    document.body.append(carryEl);
  };
  ...
  const positionCarry = (e: PointerEvent) => {
    ...
    if (carryEl.className !== cls) {
      carryEl.className = cls;
    }
  };
  ...
  const updateHint = (e: PointerEvent) => {
    ...
    if (!hintEl) {
      ...
      hintEl.dataset.slot = "event-calendar-drop-hint";
      ...
      // in RTL documents
      hintEl.className = `bg-background text-destructive border-destructive/40 pointer-events-none fixed top-0 left-0 z-100 rounded-md border px-2 py-0.5 text-xs font-medium shadow-sm${
  ...
  const onPointerUp = (e: PointerEvent) => {
    ...
    if (kind === "create") {
      ...
      if (draft) {
        api.select({
          slot: { allDay: draft.allDay, end: draft.end, start: draft.start },
        });
...
function useEventCalendarGestures<TData = unknown>() {
  ...
  const beginCreate = useCallback(
    (e: React.PointerEvent, day: Date, allDay: boolean) => {
      ...
      if (!instance.getState().interactions.selectSlot) {
        return;
      }
      beginGesture({
        createAllDay: allDay,
  ...
  return { beginCreate, beginMove, beginResize, canDrag, canResize };
}
