...
function EventCalendarTimeGrid({
  ...
  const days = useMemo(() => {
    ...
    if (effective.weekends || view === "day") {
      return result;
    }
    return result.filter((day) => ![0, 6].includes(toZoned(day, settings.timeZone).getDay()));
  ...
  const allDaySegments = useEventCalendarSelector<unknown, EventCalendarSegment[]>(
    () => {
      if (!viewConfig.renderAllDaySection) {
        return EMPTY_ALL_DAY_SEGMENTS;
      }
      const { byDay } = instance.internals.getIndex();
...
function EventCalendarDayHeader({ day, view }: { day: Date; view: CalendarView }) {
  ...
  return (
    ...
    >
      {viewConfig.renderDayHeader?.({ day, isToday, view }) ??
...
function useEventCalendarAllDayBars(days: Date[]): {
  ...
  return useEventCalendarSelector<unknown, { bars: EventCalendarSegment[]; laneCount: number }>(
    () => {
      ...
      const packed = [...merged.values()].toSorted(
        (a, b) =>
...
function EventCalendarAllDayBars({
  ...
  >(
    (state) => {
      const { drag } = state;
      if (!drag || !drag.proposedDayGranular) {
        return null;
      }
    ...
    <div
      ...
      onClick={(e) => {
        if (e.target === e.currentTarget && !wasRecentDrag() && !wasRecentChipPress()) {
          settings.onSlotClick?.({ allDay: true, date: dayStart, view }, e);
        }
      }}
    ></div>
  );
...
function EventCalendarDayColumn({
  ...
  return (
    <div
      ...
      onPointerDown={(e) => {
        if (e.target === e.currentTarget) {
          gestures.beginCreate(e, day, false);
        }
      }}
      onClick={(e) => {
        if (e.target !== e.currentTarget || wasRecentDrag() || wasRecentChipPress()) {
