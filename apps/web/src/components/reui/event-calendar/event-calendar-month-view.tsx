...
function EventCalendarMonthWeek({
  ...
  >(
    (state) => {
      ...
      if (startDayMs >= rowEndMs || lastDayMs < rowStartMs) {
        return null;
      }
...
function EventCalendarMonthCell({
  ...
  const inDraft = useEventCalendarSelector<unknown, { isStart: boolean; isEnd: boolean } | null>(
    (state) => {
      ...
      if (draft.start >= dayEnd || draft.end <= dayStart) {
        return null;
      }
      return {
  ...
  >(
    (state) => {
      const { drag } = state;
      if (!drag || drag.kind !== "move" || drag.proposedAllDay) {
        return null;
      }
      if (drag.proposedEnd.getTime() - drag.proposedStart.getTime() > 86_400_000) {
        return null;
      }
      const dropDayMs = zonedStartOfDay(drag.proposedStart, settings.timeZone).getTime();
      if (dropDayMs !== dayStart.getTime()) {
        return null;
      }
      if (
      ...
      return {
        color: drag.occurrence.event.color,
        min: (drag.proposedStart.getTime() - dropDayMs) / 60_000,
        valid: drag.valid,
      };
    },
    {
      ...
      >
        {viewConfig.showDayAddButton && !isInteracting && (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              settings.onSlotClick?.({ allDay: true, date: day, view: "month" }, e);
            }}
          >
  ...
  return (
    <div
      ...
      onPointerDown={(e) => {
        ...
        if (target.closest("[data-slot=event-calendar-event]")) {
          return;
        }
        if (target.closest("[data-slot=event-calendar-more]")) {
          return;
        }
        gestures.beginCreate(e, day, true);
      }}
      onClick={(e) => {
        if (wasRecentDrag() || wasRecentChipPress()) {
          return;
        }
        settings.onSlotClick?.({ allDay: true, date: day, view: "month" }, e);
      }}
...
function EventCalendarMoreIndicator({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        {viewConfig.renderMoreIndicator?.({ count, day, segments }) ??
