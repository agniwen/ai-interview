...
function EventCalendarResourceColumn({
  ...
  >(
    (state) => {
      ...
      if (targetResourceId !== resource.id) {
        return null;
      }
      const from = Math.max(
        (drag.proposedStart.getTime() - dayStart.getTime()) / 60_000,
        boundsStartMin,
      );
      const to = Math.min((drag.proposedEnd.getTime() - dayStart.getTime()) / 60_000, boundsEndMin);
      if (to <= from) {
        return null;
      }
      return {
  ...
  const draftWindow = useEventCalendarSelector<unknown, [number, number] | null>(
    (state) => {
      ...
      const from = Math.max((draft.start.getTime() - dayStart.getTime()) / 60_000, boundsStartMin);
      const to = Math.min((draft.end.getTime() - dayStart.getTime()) / 60_000, boundsEndMin);
