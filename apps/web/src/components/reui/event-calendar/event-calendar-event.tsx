 ...
 */
const EVENT_CALENDAR_COLORS: { name: string; value: string }[] = [
...
function EventCalendarEvent<TData = unknown>({
  ...
  const agendaTimeText = (() => {
    ...
    if (occurrence.allDay) {
      return settings.i18n.labels.allDay;
    }
  ...
  const agendaDefaultContent = (
    <>
      ...
      {occurrence.isRecurring && (
        <IconRepeat className="text-muted-foreground size-2.5 shrink-0" aria-hidden="true" />
      )}
    </>
  );
  ...
  const customContent = useMemo(() => {
    const renderProps = { isDragging, isSelected, occurrence, segment, view };
    return view === "agenda"
  ...
  const defaultProps = {
    ...
    onClick: (e: React.MouseEvent) => {
      ...
      if (e.defaultPrevented || view === "agenda") {
        return;
      }
