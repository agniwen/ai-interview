...
const bodyCellTruncateWrapVariants = ({ size }: { size?: "dense" | "default" }) =>
  size === "dense" ? "-mx-2 -my-1.5 truncate px-2 py-1.5" : "-mx-3 -my-2 truncate px-3 py-2";
  ...
  return (
    <th
      ...
      style={{
        ...
        ...(props.tableLayout?.columnsPinnable && isPinned && getPinningStyles(column)),
      ...
      className={cn(
        "text-foreground relative h-10 whitespace-nowrap text-left align-middle font-medium rtl:text-right [&:has([role=checkbox])]:pe-0",
        headerCellSpacing,
        props.tableLayout?.headerBackground && "bg-muted",
        props.tableLayout?.cellBorder && "border-e",
        props.tableLayout?.cellBorder && isFirstEndPinned && "border-s",
        // The end-pinned boundary is painted by the pinned column itself so
        // it remains visible over scrolled content; suppress the center
        // column's matching edge to keep that boundary at one pixel.
        props.tableLayout?.cellBorder &&
          !isPinned &&
          column.getIsLastColumn("center") &&
          (props.tableLayout?.columnsResizable || hasDataGridTableRightPinnedColumns(table)) &&
          "border-e-0",
        props.tableLayout?.columnsResizable &&
          column.getCanResize() &&
        ...
        props.tableLayout?.columnsPinnable &&
          isPinned &&
          cn(
            props.tableLayout?.headerBackground
              ? "data-pinned:bg-muted"
              : "data-pinned:bg-background",
            "data-outer-pinned-col:bg-clip-padding data-pinned:isolate",
...
function DataGridTableBodyRow<TData extends object>({
  ...
  const bodyRowBottomBorderClasses =
    "[&:not(:last-child)>td]:border-b [tbody:has(+tfoot)_&:last-child>td]:border-b";
  ...
  const renderRow = (isSelected: boolean) => (
    <tr
      ...
      className={cn(
        ...
        props.tableLayout?.rowsPinnable && isRowPinned && props.tableClassNames?.rowPinned,
        props.tableClassNames?.bodyRow,
      )}
    >
      {children}
  ...
  const rowSelectionAtom = row.table.atoms.rowSelection;
  if (!table.options.enableRowSelection || rowSelectionAtom == null) {
    return renderRow(false);
  }

  return (
    <Subscribe source={rowSelectionAtom} selector={() => row.getIsSelected()}>
      {(isSelected) => renderRow(isSelected)}
    </Subscribe>
  );
}
  ...
  const renderTd = (selection: ReturnType<typeof getDataGridCellSelectionCellAttrs> | null) => (
    <td
      ...
      className={cn(
        ...
        props.tableLayout?.cellBorder && "border-e",
        props.tableLayout?.cellBorder && isFirstEndPinned && "border-s",
        // Mirror of the head cell rule: the end-pinned column owns this
        // shared boundary, including while horizontal content scrolls below.
        props.tableLayout?.cellBorder &&
          !isPinned &&
          column.getIsLastColumn("center") &&
          (props.tableLayout?.columnsResizable || hasDataGridTableRightPinnedColumns(table)) &&
  ...
  return (
    <Fragment>
      <DataGridTableBodyRow
        ...
        dataIndex={rowIndex}
      >
        {startVisibleCells.map((cell: Cell<DataGridFeatures, TData, unknown>) => (
  ...
  return (
    <Subscribe
      ...
      selector={() => {
        ...
        return {
          checked,
          indeterminate: table.getIsSomePageRowsSelected() && !checked,
        };
      }}
    >
      {({ checked, indeterminate }) => (
...
const MemoizedDataGridTableBodyRows = memo(DataGridTableBodyRows, (prev, next) => {
  if (next.table.state.columnResizing.isResizingColumn || next.table._isSelectingCells === true) {
    return true;
  }

  // Row selection has its own atom subscribers. When the row model is
  // unchanged, reconciling the complete body would only re-run every cell
  // renderer; the affected checkbox and row tint update independently.
  return (
    prev.table.store === next.table.store &&
    prev.table.options.columns === next.table.options.columns &&
    prev.table.options.data === next.table.options.data &&
    prev.table.state.rowSelection !== next.table.state.rowSelection &&
    prev.table.getRowModel() === next.table.getRowModel()
  );
