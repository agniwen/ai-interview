...
describe("animation motion regressions", () => {
  it("keeps the complete Web and Desktop motion scales identical", () => {
    const webStyles = readSource("apps/web/src/styles/globals.css");
    const desktopStyles = readSource("apps/desktop/src/renderer/src/assets/main.css");
    const properties = [
      "--duration-stagger",
      "--duration-micro",
      "--duration-quick",
  ...
  it("keeps skeleton reveal orchestration identical on Web and Desktop", () => {
    const webReveal = readSource("apps/web/src/components/ui/skeleton-reveal.tsx");
    const desktopReveal = readSource(
      "apps/desktop/src/renderer/src/components/ui/skeleton-reveal.tsx",
    );
  ...
  it("honors the operating-system reduced-motion preference in both Motion roots", () => {
    const webRoot = readSource("apps/web/src/routes/__root.tsx");
    const desktopRoot = readSource("apps/desktop/src/renderer/src/main.tsx");
  ...
  it("keeps camera tiles physical and reduced-motion aware", () => {
    const tileView = readSource(
      "apps/web/src/components/agents-ui/blocks/agent-session-view-01/components/tile-view.tsx",
    );
  ...
  it("keeps the text shimmer on CSS with a static reduced-motion state", () => {
    const shimmer = readSource("apps/web/src/components/ai-elements/shimmer.tsx");
    const globalStyles = readSource("apps/web/src/styles/globals.css");
  ...
  it("keeps shared surface motion on semantic tokens with reduced-motion exits", () => {
    const globalStyles = readSource("apps/web/src/styles/globals.css");
    const cossStyle = readSource("apps/web/src/components/ui/coss-style.ts");
    const dialog = readSource("apps/web/src/components/ui/dialog.tsx");
    const alertDialog = readSource("apps/web/src/components/ui/alert-dialog.tsx");
    const modal = readSource("apps/web/src/components/ui/modal.tsx");
    const sheet = readSource("apps/web/src/components/ui/sheet.tsx");
    const tabs = readSource("apps/web/src/components/ui/tabs.tsx");
  ...
  it("does not remount dialog roots when their open state or target changes", () => {
    const launchInterview = readSource(
      "apps/web/src/components/features/studio/resumes/launch-interview-dialog.tsx",
    );
    const formTemplates = readSource(
      "apps/web/src/components/features/studio/forms/form-template-management-page.tsx",
    );
    const mailAccounts = readSource(
      "apps/web/src/components/features/platform/mail-ingest-accounts/mail-ingest-accounts-grid.tsx",
    );
    const offerStage = readSource("apps/web/src/components/features/studio/offer-stage-panel.tsx");
    const workspacePermissions = readSource(
      "apps/web/src/components/features/studio/members/workspace-permissions-section.tsx",
    );
    const interviewQuestionTemplates = readSource(
      "apps/web/src/components/features/studio/interview-questions/interview-question-template-management-page.tsx",
    );
    const jobDescriptions = readSource(
      "apps/web/src/components/features/studio/job-descriptions/job-description-management-page.tsx",
    );
  ...
  it("uses the same interaction-gated toggle recipe on Web and Desktop", () => {
    const webSwitch = readSource("apps/web/src/components/ui/switch.tsx");
    const desktopSwitch = readSource("apps/desktop/src/renderer/src/components/ui/switch.tsx");
    const webStyles = readSource("apps/web/src/styles/globals.css");
  ...
  it("animates shared progress through transforms instead of width", () => {
    const progress = readSource("apps/web/src/components/ui/progress.tsx");
  ...
  it("preserves the original coordinated desktop sidebar transition", () => {
    const sidebar = readSource("apps/web/src/components/ui/sidebar.tsx");
  ...
  it("signals when animated height changes finish", () => {
    const animatedHeight = readSource(
      "apps/web/src/components/features/motion/animated-height.tsx",
    );
