...
- **Monorepo**: Bun workspace + Turborepo at the root. All workspace packages use the `@app/*` scope. This includes app-owned runtimes and tools, plus stable cross-application contracts such as `@app/shared` (shared types, schemas, and isomorphic utilities) and `@app/db-schema` (Drizzle schema/relations + DB-adjacent shared types).
...
**Drizzle schema lives in the `@app/db-schema` workspace package**, not under `src/lib/`. The package exports `schema`, `relations`, and DB-adjacent shared types (`candidate-forms`, `db-enums`, `interview-question-templates`, `interview-session`, `interview/types`, `job-description-config`, `minimax-voices`, `studio-interviews`, `resume-parser-schema`) — anything imported by `schema.ts`. Import as `@app/db-schema/schema`, `@app/db-schema/relations`, `@app/db-schema/candidate-forms`, etc. The actual DB connection lives in `@app/server/lib/server/db` and imports `relations` from the package. `drizzle.config.ts` points at `../../packages/db-schema/src/schema.ts`.

When a module _mostly_ fits `@app/shared` but has one backend-only function (e.g. `hashTemplateSnapshot` using `node:crypto`), extract that function into a sibling `*-hash.ts` (or similar) under `@app/server/lib/server/` and keep the rest in `@app/shared`. Don't pull `node:*`, TanStack Start request helpers, or app-local `@/` imports into `packages/shared/src`.
