/* oxlint-disable jsdoc/check-tag-names -- `@jsxImportSource` on next line is a TS compiler directive, not a JSDoc tag. */
    ...
    <Card title={titleParts.join(" · ")}>
      <Section>
        <Fields>
          ...
          <Field
            label="评分"
            value={score === null || score === undefined ? "待核实" : `${score} / 100`}
          />
      ...
      <Divider />
      <Section>
        <CardText>{`**✨ 候选人优点**\n${joinList(strengths)}`}</CardText>
      </Section>
      <Section>
        <CardText>{`**⚠️ 关键风险项**\n${joinList(risks)}`}</CardText>
      </Section>
      {followUps && followUps.length > 0 ? (
        <Section>
          <CardText>{`**❓ 建议追问问题**\n${joinList(followUps)}`}</CardText>
        </Section>
      ) : null}
    </Card>
  );
