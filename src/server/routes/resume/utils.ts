...
const JOB_DESCRIPTION_PRESETS: {
  keywords: RegExp;
  jd: (context: JobDescriptionContext) => string;
}[] = [
  {
    jd: (context) =>
      buildPresetJobDescription(
        context,
        "保障部门日常行政与协同流程高效运转",
        [
        ...
        ["有行政助理、学生组织事务或活动统筹经验", "熟悉流程管理、采购协同或费用报销场景"],
      ),
    keywords: /行政/,
  },
  {
  ...
  },
  {
    jd: (context) =>
      buildPresetJobDescription(
        ...
        ["有中后台、增长、内容、电商或 AI 产品经验", "熟悉 Axure、Figma、SQL 或埋点分析工具"],
      ),
    keywords: /产品/,
  },
  {
...
export function buildAutoJobDescription(role: string): string {
  ...
  return buildPresetJobDescription(
    ...
    ["有相关项目、实习、校园实践或作品经历", "能适应互联网团队较快节奏，有较强自驱力与协作意识"],
  );
}
...
function buildResumeSelectorMissResult(availableResumeNames: string[], resumeName?: string) {
  return {
    count: 0,
...
export function createExtractResumePdfTextTool({
  ...
  return tool({
    ...
    execute: async ({ maxChars, resumeName }) => {
      ...
      const textLimit = maxChars ?? 12_000;
