...
function buildParsedResumeTextBlock(parsed: ResumeParsedPartData): string {
  const block = [
    ...
    "以下结构化信息已通过 Qwen-VL OCR + DeepSeek V4 Flash 提取完毕，可直接使用，**无需再调用任何 PDF 解析工具**。",
    "结构化信息（JSON）:",
    "```json",
...
【🔴 简历筛选执行顺序（强制）】
当本轮包含简历 PDF 且自动解析块有效（结构化字段中 name / skills / projectExperiences / workExperiences / timelineSummary 至少 2 项非空）时，必须按以下顺序执行，**不得跳步**：
  ...
  步骤 0 — JD 前置检查（仅在【无在招 JD】时执行；若已有 JD 直接跳到步骤 1）：
    ...
    满足"无在招 JD"时**必须**执行：
      ...
      2. 若返回 status === 'ok'：**立即**调用 apply_job_description，原样转发 candidates / recommendedId / reasoning，并停止本轮输出，等待用户在 UI 上点击确认或忽略。
      3. 若返回 status === 'no-jds' / 'no-resume' / 'error'：跳过 apply，简短告知用户"后台暂无可推荐岗位"或"推荐不可用"，再进入步骤 1。
