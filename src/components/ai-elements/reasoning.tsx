...
export const ReasoningTrigger = memo(
  ({
    ...
    return (
      <CollapsibleTrigger
        className={cn(
          "flex w-full items-center gap-2 text-muted text-sm transition-colors hover:text-foreground",
          className,
        )}
        {...props}
      >
...
export const ReasoningContent = memo(({ className, children, ...props }: ReasoningContentProps) => (
  <CollapsibleContent
    className={cn(
      "mt-4 text-sm",
      "data-[state=closed]:fade-out-0 data-[state=closed]:slide-out-to-top-2 data-[state=open]:slide-in-from-top-2 text-muted outline-none data-[state=closed]:animate-out data-[state=open]:animate-in",
      className,
    )}
    {...props}
  >
    {/* @ts-expect-error ignore */}
    <Streamdown plugins={streamdownPlugins} {...props} dir="auto">
      {children}
    </Streamdown>
  </CollapsibleContent>
));
