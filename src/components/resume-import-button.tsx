          ...
          <div className='flex items-center gap-1.5' key={item.key}>
            <span
              className={cn(
                ...
                done && 'border-emerald-400/70 bg-emerald-50 text-emerald-600 dark:border-emerald-700/60 dark:bg-emerald-950/40 dark:text-emerald-300',
  ...
  async function runImport() {
    ...
    try {
      ...
      if (!questionsResponse.ok) {
        const errBody = await questionsResponse.json().catch(() => null) as { error?: string } | null;
      ...
      await readNdjsonStream<AnalysisStreamEvent>(questionsResponse, (event) => {
        handleStreamEvent(event);
        if (event.type === 'result') {
      ...
      saveForm.append(
        'scheduleEntries',
        JSON.stringify([
          { notes: '', roundLabel: '一面', scheduledAt: '', sortOrder: 0 },
        ]),
      );
      ...
      if (!saveResponse.ok || !savedPayload || 'error' in savedPayload) {
        throw new Error(
          savedPayload && 'error' in savedPayload
            ? savedPayload.error ?? '保存失败'
            : '保存失败',
        );
      }
