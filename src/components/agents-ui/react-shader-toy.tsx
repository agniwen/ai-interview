...
const IMAGE_URL_REGEX = /(\.jpg|\.jpeg|\.png|\.gif|\.bmp)$/i;
const VIDEO_URL_REGEX = /(\.mp4|\.3gp|\.webm|\.ogv)$/i;
const TEXTURE_CALL_REGEX = /texture\(/g;
...
function uniformTypeToGLSLType(t: string) {
  switch (t) {
    case "1f": {
      return "float";
    }
...
const MirroredRepeatWrapping = 33_648;
const ClampToEdgeWrapping = 33_071;
const RepeatWrapping = 10_497;
...
class Texture {
  ...
  load = async (textureArgs: TextureParams) => {
    ...
    const isImage = IMAGE_URL_REGEX.exec(url);
    const isVideo = VIDEO_URL_REGEX.exec(url);
    if (isImage === null && isVideo === null) {
      throw new Error(log(`Please upload a video or an image with a valid format (url: ${url})`));
    }
    Object.assign(this, { flipY, magFilter, minFilter, url, wrapS, wrapT });
    const level = 0;
    const internalFormat = gl.RGBA;
    const width = 1;
    const height = 1;
...
export function ReactShaderToy({
  ...
  const uniformsRef = useRef<
    ...
    >
  >({
    [UNIFORM_TIME]: { isNeeded: false, type: "float", value: 0 },
    [UNIFORM_TIMEDELTA]: { isNeeded: false, type: "float", value: 0 },
    [UNIFORM_DATE]: { isNeeded: false, type: "vec4", value: [0, 0, 0, 0] },
    [UNIFORM_MOUSE]: { isNeeded: false, type: "vec4", value: [0, 0, 0, 0] },
    [UNIFORM_RESOLUTION]: { isNeeded: false, type: "vec2", value: [0, 0] },
    [UNIFORM_FRAME]: { isNeeded: false, type: "int", value: 0 },
    [UNIFORM_DEVICEORIENTATION]: { isNeeded: false, type: "vec4", value: [0, 0, 0, 0] },
  });
  ...
  const processCustomUniforms = () => {
    if (propUniforms) {
      for (const name of Object.keys(propUniforms)) {
        ...
        uniformsRef.current[name] = { isNeeded: false, type: glslType, value, ...tempObject };
      }
    }
  };
  ...
  const preProcessFragment = (fragment: string) => {
    ...
    let fragmentShader = precisionString
      ...
      .concat(fragment.replace(TEXTURE_CALL_REGEX, "texture2D("));
