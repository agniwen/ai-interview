...
const FORMS_TOUR_STEPS: DriveStep[] = [
  {
    popover: {
      description:
        "候选人点开面试链接后、进入语音面试间之前，会被强制要求填写这里维护的表单。常用于背调、到岗时间确认、知情同意等需要前置收集的信息。提交后内容会冻结为快照，之后修改不影响历史答卷。",
      title: "面试表单",
    },
  },
  {
  ...
  },
  {
    ...
    popover: {
      description:
        "按作用范围筛选：全局面试表单会出现在所有面试中；岗位绑定的只对该在招岗位的面试生效。",
      title: "按作用域筛选",
    },
  },
  {
    ...
    popover: {
      description:
        "按在招岗位筛选，查看绑定到该岗位的所有面试表单。从「在招岗位 → 面试表单」tab 点过来时也会自动用这个参数过滤。",
      title: "按岗位筛选",
    },
  },
  {
    ...
    popover: {
      description:
        "新建一份面试表单：设标题、作用范围（全局或绑定岗位），添加题目。每题可选「单选 / 多选 / 填写」，再选展示方式（下拉、单/多选框、单/多行输入），并标注是否必填。",
      title: "新建面试表单",
    },
  },
  {
    ...
    popover: {
      description:
        "展示作用范围、题目数和已填写人数。点「已填写」数字或「更多 → 查看填写记录」可以看到该面试表单下所有候选人的提交快照。",
      title: "面试表单列表",
    },
  },
  {
    popover: {
      description:
        "想重置某位候选人的某份面试表单？去「面试库 → 候选人详情 → 表单答复」tab，点对应卡片的「重置填写」即可，候选人下次进面试链接会被要求重新填写这一份。如果轮次已结束，还需在「面试安排」里重置轮次。",
      title: "重置候选人答卷",
    },
  },
];
