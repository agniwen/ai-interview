...
import type * as S3Module from "@arc/backend/lib/server/s3";
import { getObjectStream } from "@arc/backend/lib/server/s3";
import type * as ResumeAgentModule from "@arc/backend/server/agents/resume-analysis-agent";
...
import { insertBatchWithItems } from "@arc/backend/server/routes/studio/routes/resume-upload-batches/dao/batches";
import { processNextItem } from "@arc/backend/server/routes/studio/routes/resume-upload-batches/utils/processor";

vi.mock("@arc/backend/lib/server/s3", async () => {
  const actual = await vi.importActual<typeof S3Module>("@arc/backend/lib/server/s3");
  return {
    ...actual,
...
vi.mock("@arc/backend/server/agents/resume-analysis-agent", async () => {
  const actual = await vi.importActual<typeof ResumeAgentModule>(
    "@arc/backend/server/agents/resume-analysis-agent",
  );
  return {
    ...actual,
