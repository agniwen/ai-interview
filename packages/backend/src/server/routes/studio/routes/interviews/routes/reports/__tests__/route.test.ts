...
vi.mock("@arc/backend/server/middlewares/permission", () => ({
  requirePermission: () => (_c: unknown, next: () => Promise<void>) => next(),
}));

vi.mock("@arc/backend/server/routes/studio/routes/interviews/dao/interview-conversations", () => ({
  queryInterviewConversationReportsByRound: mocks.queryInterviewConversationReportsByRound,
}));

vi.mock("@arc/backend/server/routes/studio/routes/interviews/dao/interview-rounds", () => ({
