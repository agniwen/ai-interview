...
`packages/backend/src/server/agents/chat-models.config.ts` 是聊天可选模型的**唯一可信源**。
...
| 文件                                                                                             | 检查点                                                                                                             |
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| `packages/backend/src/server/agents/chat-models.config.ts`                                       | `LOCAL_CHAT_MODELS` + `LOCAL_DEFAULT_MODEL_ID`                                                                     |
...
| `packages/backend/src/server/agents/resume-agent.ts`                                             | `process.env.ALIBABA_MODEL ?? "..."` 的硬编码 fallback 是否还能用（**简历筛选 agent 专用**，跟 picker 是独立通路） |
