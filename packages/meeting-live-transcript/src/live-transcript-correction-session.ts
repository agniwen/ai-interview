...
export interface LiveTranscriptCorrectionPeer {
  baseUrl: string;
...
export function createLiveTranscriptCorrectionSession(fetch?: typeof globalThis.fetch) {
  const peers = new Map<string, LiveTranscriptCorrectionPeer>();
  const worker = createLiveTranscriptCorrection({ fetch });
  let history: { id: string; text: string; corrected?: boolean }[] = [];
  return {
    add: (peer: LiveTranscriptCorrectionPeer) => peers.set(peer.sectionId, peer),
