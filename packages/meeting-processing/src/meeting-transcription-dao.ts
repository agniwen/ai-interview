...
import type { FinalTranscriptionAudioChunk } from "./meeting-transcription-provider";
import {
...
export interface MeetingTranscriptionDaoDependencies {
  isWorkspaceAdministrator: (role: string) => boolean;
...
export interface MeetingTranscriptionPolicySnapshot {
...
export function createMeetingTranscriptionDao(
  db: Database,
  dependencies: MeetingTranscriptionDaoDependencies,
) {
  ...
  function toMeetingTranscriptionPolicySnapshot(
    ...
    return {
      ...
      fallbackProvider: meetingTranscriptionProviderSchema
        .nullable()
        .parse(policy.fallbackProvider),
      revision: policy.revision,
      selectedProvider: meetingTranscriptionProviderSchema
        .nullable()
        .parse(policy.selectedProvider),
  ...
  async function loadMeetingTranscriptionPolicy(
  ...
  async function updateMeetingTranscriptionPolicy(input: {
    actorId: string;
    organizationId: string;
    ...
    return await db.transaction(async (tx) => {
      const [activeMembership] = await tx
        ...
        .for("share");
      if (!activeMembership || !dependencies.isWorkspaceAdministrator(activeMembership.role)) {
        return null;
      }
      const [row] = await tx
  ...
  const DEFAULT_MEETING_TRANSCRIPTION_POLICY_REASON = "未配置转录策略时默认使用百炼 Qwen ASR";
   ...
   */
  async function ensureDefaultMeetingTranscriptionPolicy(
    organizationId: string,
  ...
  function loadMeetingTranscriptionSource(input: MeetingTranscriptionJobData) {
  ...
  async function loadMeetingTranscriptionChunkCheckpoint(
    input: MeetingTranscriptionJobData,
    chunk: FinalTranscriptionAudioChunk,
  ...
  async function saveMeetingTranscriptionChunkCheckpoint(
    input: MeetingTranscriptionJobData & { processingRunId: string },
    chunk: FinalTranscriptionAudioChunk,
  ...
  async function markMeetingTranscriptionChunkFailed(
    input: MeetingTranscriptionJobData & { processingRunId: string },
    chunk: FinalTranscriptionAudioChunk,
  ) {
    await db
  ...
  async function claimMeetingTranscriptionRun(
  ...
  async function markMeetingTranscriptionFailed(
    input: MeetingTranscriptionJobData & {
  ...
  async function publishMeetingTranscript(
    input: MeetingTranscriptionJobData & {
      processingRunId: string;
      transcript: CanonicalMeetingTranscript;
      warning?: string;
  ...
  async function resetMeetingTranscriptionForRetry(input: {
    meetingId: string;
    organizationId: string;
  }) {
    return await db.transaction(async (tx) => {
