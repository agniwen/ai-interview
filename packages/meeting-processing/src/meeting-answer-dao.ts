...
export function createMeetingAnswerDao(
  db: Database,
  ...
  async function claimMeetingAnswerExchange(input: {
    attempt: number;
    exchangeId: string;
    executionToken: string;
  }): Promise<AnswerClaimResult> {
  ...
  async function loadMeetingAnswerContext(input: { exchangeId: string; executionToken: string }) {
    return await db.transaction(async (tx) => {
  ...
  async function publishMeetingAnswerExchange(input: {
  ...
  async function markMeetingAnswerFailed(input: {
    exchangeId: string;
    executionToken: string;
    terminal: boolean;
  }): Promise<boolean> {
  ...
  async function listRecoverableMeetingAnswerJobs(): Promise<MeetingAnswerJobData[]> {
    return await db
      .select({ exchangeId: meetingQuestionExchange.id })
      .from(meetingQuestionExchange)
      .innerJoin(meetingSession, eq(meetingSession.id, meetingQuestionExchange.meetingId))
      .where(
        and(
          eq(meetingSession.processingOwner, processingOwner),
          or(
...
export type MeetingAnswerClaim = Awaited<
  ReturnType<ReturnType<typeof createMeetingAnswerDao>["claimMeetingAnswerExchange"]>
>;
